...
def test_quality_ignores_empty_pcm(railway_mod):
    railway_mod._quality("admin", "spk", b"")
...
def test_quality_odd_byte_pcm_does_not_crash(railway_mod):
    ...
    railway_mod._quality("admin", "spk", b"\x01\x02\x03")
...
def test_loud_tone_reads_good(railway_mod):
    ...
    railway_mod._quality("admin", "spk", pcm16(tone))
    q = railway_mod.QUALITY[("admin", "spk")]
    assert q["level"] > 0.01 and q["speech_sec"] > 0
    assert railway_mod._quality_verdict(q) == "good"


def test_clipping_detected(railway_mod):
    q = railway_mod.QUALITY.setdefault(("admin", "spk"), {"level": 0.5, "peak": 0.0, "clips": 0,
                                                          "speech_sec": 10.0, "total_sec": 10.0})
    railway_mod._quality("admin", "spk", pcm16([0.999, -0.999] * 8000))  # full-scale -> peak~1
    assert railway_mod.QUALITY[("admin", "spk")]["peak"] >= 0.985
    assert railway_mod._quality_verdict(railway_mod.QUALITY[("admin", "spk")]) == "CLIPPING — lower volume"


def test_warming_up_under_3s(railway_mod):
    railway_mod._quality("admin", "spk", pcm16([0.3] * 16000))  # 1 s only
    assert railway_mod._quality_verdict(railway_mod.QUALITY[("admin", "spk")]) == "warming up"


def test_total_sec_accumulates(railway_mod):
    for _ in range(3):
        railway_mod._quality("admin", "spk", pcm16([0.2] * 16000 * 2))  # 2 s each
    assert railway_mod.QUALITY[("admin", "spk")]["total_sec"] == 6.0
